...
```{r Read data and split into trials, echo=FALSE, warning=FALSE}
...
```{r Forest plots, warning=FALSE, echo=FALSE}
...
```{r Compute ratios, warning=FALSE, echo=FALSE}
...
#Once all the non usable instances have been removed, compare log(HR)s of the sensitivity analyses to the one of the primary one.
 if(!is.na(ref_HR) & length(sens_HR)!=0){ #Check that there are sensitivity analyses to compare
    ...
    if(nrow(sens_type)>0){
      ...
      NCT_comp[[end]]=data.frame(log_ratio=ratio_HR, log_se=ratio_se)
      }
  }
...
ggplot(f_long, aes(x = Category, y = Count, fill = Endpoint)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("OS" = "dodgerblue", "TBE" = "seagreen3"))+
  labs(#title = "OS and TBE per Category",
...
ggplot(f_long_rel, aes(x = Category, y = Count, fill = Endpoint)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
#### Different categories of sensitivity analyses in the same study
```{r}
library(dplyr)
...
ggplot(f_long, aes(x = Category, y = Count, fill = Endpoint)) +
  ...
  labs(#title = "Number of category of sensitivity analysis in the same study",
       x = "Number of categories",y = "Number of studies") +
  #labs(x = "Category",y = "Frequency (%)") +
...
```{r Differences in HR, warning=FALSE}
...
```
```{r}
ggplot(df, aes(x = endpoint, y = exp(log_ratio), fill = endpoint)) +
  geom_boxplot() +
   scale_fill_manual(
    name = "endpoint", 
    values = c("OS" = "dodgerblue", "TBE" = "seagreen3"),
