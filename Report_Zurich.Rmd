...
```{r Combine comparisons, warning=FALSE}
dif_OS=data.frame()
dif_TBE=data.frame()
x=y=0
...
```{r}
table(dif_TBE_clean$Type.of.sensitivity.analysis)
table(dif_TBE_clean$Censoring)
table(dif_TBE_clean$Methods)
table(dif_TBE_clean$Population)

table(dif_OS_clean$Type.of.sensitivity.analysis)
table(dif_OS_clean$Censoring)
table(dif_OS_clean$Methods)
table(dif_OS_clean$Population)
...
```{r Differences in Se, warning=FALSE}
...
```{r Scatter plot TBE, fig.width=7, fig.height=5, warning=FALSE}
...
```{r Scatter plot OS, fig.width=7, fig.height=5, warning=FALSE}
...
```{r Plot only Methods data, include=FALSE}
plot_data <- component_data_TBE %>%
  filter(Methods == "Yes")
...
```{r Plot only Population data, include=FALSE}
plot_data <- component_data_TBE %>%
...
```{r Compbine subplot TBE, fig.width=10}
library(ggplot2)
library(dplyr)
...
```{r Combine subplot OS, fig.width=10}
library(tidyr)
library(dplyr)
...
```{r Random model TBE, warning=FALSE}
...
```{r Factor covariates OS, warning=FALSE}
