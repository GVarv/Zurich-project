...
#ggplot(pip_long_facet, aes(x = Group, y = PIP, fill = Group)) + geom_boxplot(width = 0.6, outlier.shape = NA, alpha = 0.4, position = position_dodge(0.8)) + geom_quasirandom(aes(color = Group), width = 0.15, alpha = 0.6, size = 1.5, dodge.width = 0.8) + scale_fill_manual(values = c("Yes" = "blue", "No" = "red")) + scale_color_manual(values = c("Yes" = "blue", "No" = "red")) + facet_grid(Component ~ Variable, scales = "free_x") + labs(x = "", y = "Posterior Probability", title = "Posterior Probabilities by Study Variables and Components") + theme_minimal() + theme( strip.text = element_text(face = "bold"), axis.text.x = element_text(angle = 0, hjust = 0.5) )
```
