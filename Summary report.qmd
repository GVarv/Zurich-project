...
```{r}
#| eval: false
# Step by step to see where it breaks

# Step 1: check distinct pairs
step1 <- dif_TBE_clean %>%
  distinct(NCT, Type.of.sensitivity.analysis)
print(step1)

# Step 2: check collapse per NCT
step2 <- step1 %>%
  group_by(NCT) %>%
  summarise(
    n_categories = n(),
    categories = paste(sort(Type.of.sensitivity.analysis), collapse = " + ")
  )
print(step2)
...
step2 %>% filter(NCT == "NCT03734016")  # replace with a known multi-category NCT
```
...
component_data_TBE <- bind_cols(
  dif_TBE_clean %>% select(NCT, se, log_ratio,Methods, Censoring, Population),
  post_probs_df_TBE,
  component_ids_TBE
)%>%
...
ggplot(plot_data,
       aes(x = log_ratio, y = se, color = Component)) +
  geom_point(size = 2.5, alpha = 0.8) +
  theme_minimal() +
  labs(title = "SE vs Log Ratio (Censoring = Yes)",
       x = "Log Ratio",
       y = "SE",
       color = "Component") +
...
ggplot(plot_data,
  ...
  geom_point(size = 2.5, alpha = 0.8) +
  theme_minimal() +
  labs(title = "SE vs Log Ratio (Methods = Yes)",
       x = "Log Ratio",
       y = "SE",
       color = "Component") +
  scale_color_manual(values = c("1" = "steelblue",
...
ggplot(plot_data,
  ...
  geom_point(size = 2.5, alpha = 0.8) +
  theme_minimal() +
  labs(title = "SE vs Log Ratio (Population = Yes)",
       x = "Log Ratio",
       y = "SE",
       color = "Component") +
  scale_color_manual(values = c("1" = "steelblue",
...
component_data_OS <- bind_cols(
  dif_OS_clean %>% select(NCT, se, log_ratio,Methods, Censoring, Population),
  post_probs_df_OS, 
  component_ids_OS
)%>%
...
hse_llr_OS=dif_OS_clean[(dif_OS_clean$se>1.1 & dif_OS_clean$log_ratio<(-0.1)), ]
lse_hlr_OS=dif_OS_clean[(dif_OS_clean$se<0.9 & dif_OS_clean$log_ratio>0.1),]
lse_trials_OS=unique(lse_hlr_OS$NCT)
hse_trials_OS=unique(hse_hlr_OS$NCT)
...
```{r}
hse_llr=dif_TBE_clean[dif_TBE_clean$se>1.1 & dif_TBE_clean$log_ratio<(-0.1), ]
lse_hlr=dif_TBE_clean[dif_TBE_clean$se<0.9 & dif_TBE_clean$log_ratio>0.1,]
